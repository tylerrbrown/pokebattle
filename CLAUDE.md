...
- ~~**Faint animation plays on wrong Pokemon**: Reported by Makoo 3/20/2026. Root cause: `playJourneyEvents` damage/faint handlers used name matching (`S.oppTeam[0].name === evt.pokemon`) which broke once `updateJourneyState` had already swapped in the next opponent. Now prefers `evt.player_index`, then `evt.side`, with name match as last resort. Fixed 4/9/2026~~
- ~~**Cross-region legendaries in Johto**: All 100 Johto Pokemon (dex 152-251) were classified as "common" in pokemon.json - no legendaries, rares, or uncommons. When RNG rolled legendary rarity (or pity triggered), the empty regional pool caused a fallback to the global pool, spawning legendaries from other regions. Fixed by classifying Johto Pokemon correctly: 6 legendary (Raikou, Entei, Suicune, Lugia, Ho-Oh, Celebi), 11 rare, 25 uncommon, 58 common. Reported by Makoo. Fixed 3/24/2026~~
...
- ~~**All mega evolutions**: 27 new mega forms added (Sceptile through Diancie) with canon stats (special = max(sp_atk, sp_def)). 35 new mega stones in shop ($10K each). Mega sprite effect: purple/pink glow + scale 1.15.~~ (DONE 4/9/2026)
- ~~**Stat-changing moves**: 21 new stat moves added to `apply_stat_effect` via list-of-tuples refactor. Includes single-stat (swords-dance, nasty-plot, iron-defense, rock-polish, charm, feather-dance, metal-sound, charge, calm-mind, etc.) and multi-stat (bulk-up, dragon-dance, hone-claws, shell-smash, tickle, cosmic-power). Works in both PvP and journey paths.~~ (DONE 4/9/2026)
- ~~**Healing moves**: milk-drink, slack-off, roost, synthesis, moonlight, morning-sun, soft-boiled (50% HP heal) and refresh, heal-bell, aromatherapy (status cure). Wired into both PvP (`battle_engine.resolve_move`) and journey (`_resolve_single_move`). Journey playJourneyEvents handler gained `heal`, `stat_change`, `status_cure`, `no_effect` cases.~~ (DONE 4/9/2026)
